# (Keep all existing imports as before)
...
def render_header():
    ...
    <div class="asdp-banner"><h1>Avijit AI 2k25 – Face Emotion Detector</h1></div>
    """
...
def home_page():
    ...
    st.markdown("<h1>🏠 Welcome to Avijit AI 2k25 – Real-Time Emotion Intelligence</h1>", unsafe_allow_html=True)

    st.markdown("""
    <p>
        <strong>Avijit AI 2k25</strong> is a cutting-edge emotion recognition platform engineered for the future of intelligent interaction.
    ...
    st.markdown("<h3>🚀 Why Avijit AI 2k25 Stands Out</h3>", unsafe_allow_html=True)
    st.markdown("""
    <ul>
    ...
    if uploaded_file:
        ...
        if len(faces) == 0:
            ...
            for idx, (x, y, w, h) in enumerate(faces):
                ...
                confidence = {emotion_labels[i]: float(prediction[i]) for i in range(len(emotion_labels))}
                cv2.rectangle(result_img, (x, y), (x + w, y + h), (255, 0, 0), 2)
                cv2.putText(result_img, emotion, (x, y - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36, 255, 12), 2)
                results.append({"face_idx": idx + 1, "emotion": emotion, "confidence": confidence})
            st.image(cv2.cvtColor(result_img, cv2.COLOR_BGR2RGB), caption="Analysis Result", use_container_width=True)
            for result in results:
                st.markdown(f"**Face {result['face_idx']}: Detected Emotion - _{result['emotion']}_**")
                conf_df = pd.DataFrame({
                    "Emotion": list(result["confidence"].keys()),
                    "Confidence (%)": [round(c * 100, 2) for c in result["confidence"].values()],
                })
                st.bar_chart(conf_df.set_index("Emotion"))

# About + Careers + Contact
def about_page():
    st.markdown("<h1 style='text-align:center; color:#4A90E2;'>About This Application</h1>", unsafe_allow_html=True)
    ...
    member = [
        
        {
            ...
            "phone": "+91-8336838993",
            "email": "paulavijit@gmail.com",
            "image": "assets/avijit.jpg"
        }
       
    ]

    cols = st.columns(4)
    for col, member in zip(cols, member):
        col.image(member["image"], use_container_width=True)
        col.markdown(f"**{member['name']}**  \n*{member['role']}*  \n📞 {member['phone']}  \n📧 {member['email']}")

    st.markdown("<p style='text-align:center; color:gray;'>© 2025 Emotion Detector App — All rights reserved.</p>", unsafe_allow_html=True)
